Feature: ghub cherry-pick
  Background:
    Given I am in "git://github.com/rtomayko/ronn.git" git repo
    And I am "mislav" on github.com with OAuth token "OTOKEN"

  Scenario: Unchanged
    When I run `ghub cherry-pick a319d88`
    Then the git command should be unchanged

  Scenario: From GitHub commit URL
    When I run `ghub cherry-pick https://github.com/rtomayko/ronn/commit/a319d88#comments`
  ...
  Scenario: From fork that has existing remote
    ...
    When I run `ghub cherry-pick https://github.com/mislav/ronn/commit/a319d88`
    Then "git fetch mislav" should be run
    And "git cherry-pick a319d88" should be run

  Scenario: Using GitHub owner@SHA notation
    Given the "mislav" remote has url "git@github.com:mislav/ronn.git"
    When I run `ghub cherry-pick mislav@a319d88`
    Then "git fetch mislav" should be run
    And "git cherry-pick a319d88" should be run

  Scenario: Using GitHub owner@SHA notation that is too short
    When I run `ghub cherry-pick mislav@a319`
    Then the git command should be unchanged

  Scenario: Unsupported GitHub owner/repo@SHA notation
    When I run `ghub cherry-pick mislav/ronn@a319d88`
    Then the git command should be unchanged

  Scenario: Skips processing if `-m/--mainline` is specified
    When I run `ghub cherry-pick -m 42 mislav@a319d88`
    Then the git command should be unchanged
    When I run `ghub cherry-pick --mainline 42 mislav@a319d88`
    Then the git command should be unchanged
  ...
  Scenario: From fork that doesn't have a remote
    When I run `ghub cherry-pick https://github.com/jingweno/ronn/commit/a319d88`
